...
public class FetchTreeGenerator {
    ...
    public void addFetchClause(FetchNode root, String fetchClause) {
        ...
        for (String nodeName : nodeNames) {
            if (currentNode == null) {
                // Root node
                PersistenceMapping.EntityConnection entityConnection =
                        getEntityConnection(root.entityClass(), nodeName)
                                .orElseThrow(() -> new IllegalArgumentException("No such field: " + nodeName));
                currentNode = root.children().getOrDefault(nodeName, new FetchNode(nodeName, entityConnection.targetEntity(), new HashMap<>()));
                ...
                PersistenceMapping.EntityConnection entityConnection =
                        getEntityConnection(currentNode.entityClass(), nodeName)
                                .orElseThrow(() -> new IllegalArgumentException("No such field: " + nodeName));
                currentNode.children().put(nodeName, new FetchNode(nodeName, entityConnection.targetEntity(), new HashMap<>()));
                currentNode = currentNode.children().get(nodeName);
            }
        }
    }
        ...
        for (Map.Entry<String, FetchNode> child : node.children().entrySet()) {
            ...
            if (!isBagFetch || !bagFetched.get()) {
                ...
                if (child.getValue().children().isEmpty()) {
                    ...
                    addSubgraph(subgraph.addSubgraph(PersistenceMapping.queryStringToGraphPath(child.getKey())), child.getValue(), bagFetched);
                }
            }
        }
    }
        ...
        return Optional.of(persistenceMapping.getEntityRelationshipMap())
                ...
                .map(entityConnection ->
                        Set.of(
                                OneToMany.class,
                                ManyToMany.class
                        ).contains(entityConnection.relationshipType()))
                .orElse(false);
    }
