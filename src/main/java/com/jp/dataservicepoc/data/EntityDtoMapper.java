...
public class EntityDtoMapper {
    ...
    public <D, E> List<D> entitiesToDtos(Iterable<E> entities, Class<D> dtoClass, @Nullable Collection<String> fetchClauses) {
        Map<String, FetchNode> fetchGraph = Optional.ofNullable(fetchClauses)
                .map(this::createFetchGraph)
    ...
    private  <D, E> D entityToDto(E entity, Class<D> dtoClass, @NonNull Map<String, FetchNode> fetchGraph) {
        ...
        fetchGraph.forEach((key, fetchNode) -> {
            try {
                ...
                Method dtoSetMethod = dto.getClass().getDeclaredMethod("set" + nameToDtoName(key), dtoGetMethod.getReturnType());
                if (entityResponse instanceof Collection<?>) {
                    dtoSetMethod.invoke(dto, ((Collection<?>) entityResponse).stream().map(entityObj ->
                            entityToDto(entityObj, (Class<?>) ((ParameterizedType) dtoGetMethod.getGenericReturnType()).getActualTypeArguments()[0],
                                    fetchNode.children())).collect(Collectors.toList()));
                } else {
                    dtoSetMethod.invoke(dto, entityToDto(entityResponse, dtoGetMethod.getReturnType(), fetchNode.children()));
                }
