...
public class JPAEntityGenerator extends SqlTableFileGenerator {
    ...
    protected void generate(Table table, Collection<TableRelationship> allTableRelationships, File outputDirectory) {
        ...
        TypeSpec.Builder entityClass = TypeSpec.classBuilder(transformTableName(table.getName()));
        TypeSpec.Builder entityIdClass = null;
        entityClass.addModifiers(Modifier.PUBLIC);
        entityClass.addAnnotation(AnnotationSpec.builder(jakarta.persistence.Entity.class)
                .addMember("name", "$S", table.getName())
                .build());
        entityClass.addAnnotation(AnnotationSpec.builder(ToString.class).build());
            ...
            switch (relationship.relationshipType()) {
                    ...
                    if (columnReference.getPrimaryKeyColumn().getParent().equals(relationship.rootTable())) {
                        ...
                        fieldSpecBuilder.addAnnotation(AnnotationSpec.builder(jakarta.persistence.OneToOne.class)
                                ...
                                .addMember(
                                        ...
                                        "$S",
                                        transformColumnName(
                                                relationship.rootTable().getName(), true))
                                .addMember("optional", "$L", false)
                                .build());
                        fieldSpecBuilder.addAnnotation(
                                AnnotationSpec.builder(jakarta.persistence.PrimaryKeyJoinColumn.class)
                                        .build());
                    } else {
                        // OneToOne child side
                        fieldSpecBuilder.addAnnotation(AnnotationSpec.builder(jakarta.persistence.OneToOne.class)
                                .addMember("fetch", "$T.LAZY", jakarta.persistence.FetchType.class)
                                .build());
                        fieldSpecBuilder.addAnnotation(AnnotationSpec.builder(jakarta.persistence.MapsId.class)
                                .build());
                        ...
                        fieldSpecBuilder.addAnnotation(AnnotationSpec.builder(jakarta.persistence.JoinColumn.class)
                                .addMember(
                                        "name",
                                        "$S",
                                        columnReference.getForeignKeyColumn().getName())
                                .build());
                    }
                    break;
                }
                ...
                case MANY_TO_MANY:
                    ...
                    Table joinTable = relationship.columnReferences().stream()
                            .filter(columnReference -> columnReference
                                    .getPrimaryKeyColumn()
                                    .getParent()
                                    .equals(relationship.rootTable()))
                            .findFirst()
                            .orElseThrow()
                            .getForeignKeyColumn()
                            .getParent();
                    List<String> joinColumnNames = joinTable.getForeignKeys().stream()
                            .filter(foreignKey ->
                                    foreignKey.getPrimaryKeyTable().equals(relationship.rootTable()))
                    ...
                    List<String> inverseColumnNames = joinTable.getForeignKeys().stream()
                            .filter(foreignKey ->
                                    foreignKey.getPrimaryKeyTable().equals(relationship.targetTable()))
                            .flatMap(foreignKey -> foreignKey.getConstrainedColumns().stream())
                            .map(NamedObject::getName)
                            .toList();
                    fieldSpecBuilder.addAnnotation(AnnotationSpec.builder(jakarta.persistence.JoinTable.class)
                            ...
                            .addMember(
                                    ...
                                    joinColumnNames.stream()
                                            .map(columnName -> AnnotationSpec.builder(
        ...
        JavaFile javaFile = JavaFile.builder("com.jp.codegen.entity", entityClass.build())
                .indent("    ")
                .build();
        JavaFile entityIdJavaFile = null;
        if (entityIdClass != null) {
            entityIdJavaFile = JavaFile.builder("com.jp.codegen.entity", entityIdClass.build())
                    .indent("    ")
                    .build();
        }
        ...
        try {
            if (log.isTraceEnabled()) {
                ...
                if (entityIdJavaFile != null) {
                    entityIdJavaFile.writeTo(System.out);
                }
            }
            Path path = Paths.get(outputDirectory.toURI());
            javaFile.writeTo(path);
            if (entityIdJavaFile != null) {
                entityIdJavaFile.writeTo(path);
            }
        } catch (IOException e) {
    ...
    private String transformTableName(String tableName) {
        return "Fd" + CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, tableName);
    }

    private String transformColumnName(String columnName, boolean isEntity) {
        if (isEntity) {
            return "fd" + CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, columnName);
        }
